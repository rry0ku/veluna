...
login-problem-network = Veluna konnte Spotify nicht erreichen. Prüfe deine Internetverbindung und versuche es erneut.
...
login-problem-premium = Veluna streamt über Spotify Premium, und dieses Konto hat es nicht. Melde dich mit einem Premium-Konto an, um fortzufahren.
...
login-browser-detail = Veluna liest deine YouTube-Music-Sitzung aus dem gewählten Browser. Die Sitzung bleibt auf diesem Gerät.
...
login-usage-consent = Hilf uns zu schätzen, wie viele Menschen Veluna nutzen.
...
login-account-detail = Diese Sitzung ist bei mehreren Google-Konten angemeldet. Wähle das Konto, das Veluna verwenden soll.
...
settings-saver-detail = Bildrate der Animationen begrenzen, wenn Veluna nicht im Fokus ist, ab dem nächsten Start
...
settings-startup-detail = Der Bildschirm, mit dem Veluna startet
...
settings-language-detail = Die Sprache, die Veluna in der Oberfläche verwendet
settings-language-system = System
settings-language-search = Sprache suchen
settings-language-none = Keine Sprache gefunden
settings-typeface = Schriftart
settings-typeface-detail = Die Schrift, die Veluna in der Oberfläche verwendet
...
settings-close-to-tray-detail = Veluna bleibt nach dem Schließen im System-Tray und spielt weiter
...
settings-version-detail = Der Build von Veluna, den du verwendest
settings-license = Lizenz
settings-license-detail = GPL-3.0-Lizenz
...
settings-notice = Copyright © 2026 Veluna Contributors. Veluna kommt ohne jede Gewährleistung. Es ist freie Software, und du darfst es unter den Bedingungen der GNU General Public License v3 (GPL-3.0) weitergeben. Veluna ist inoffiziell und steht in keiner Verbindung zu Spotify AB.
...
toast-download-complete = { $name } heruntergeladen
toast-download-failed = { $name } konnte nicht heruntergeladen werden
...
update-available = Veluna { $version } ist da
update-detail = Du hast { $running }. Sieh dir an, was sich geändert hat, oder aktualisiere jetzt.
update-detail-notes = Du hast { $running }. Lade das neueste Release von GitHub herunter, um zu aktualisieren.
...
settings-check-updates-detail = Beim Start einmal bei GitHub nachfragen, ob eine neuere Version verfügbar ist
settings-check-now = Manuelle Update-Prüfung
settings-check-now-detail = Auf GitHub nach der neuesten Version von Veluna suchen
settings-check-now-btn = Jetzt prüfen
settings-checking-updates = Wird geprüft…
toast-update-available = Update verfügbar: { $name }
toast-update-up-to-date = Du bist auf dem neuesten Stand
