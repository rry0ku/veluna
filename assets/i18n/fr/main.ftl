...
login-problem-network = Veluna n'a pas pu joindre Spotify. Vérifiez votre connexion internet et réessayez.
...
login-problem-premium = Veluna diffuse via Spotify Premium, et ce compte ne l'a pas. Connectez-vous avec un compte Premium pour continuer.
...
login-browser-detail = Veluna lit votre session YouTube Music dans le navigateur choisi. La session reste sur cet appareil.
...
login-usage-consent = Aidez-nous à estimer combien de personnes utilisent Veluna.
...
login-account-detail = Cette session est connectée à plusieurs comptes Google. Choisissez celui que Veluna doit utiliser.
...
settings-saver-detail = Limiter la fréquence d'images des animations quand Veluna n'est pas au premier plan, à partir du prochain démarrage
...
settings-startup-detail = L'écran sur lequel Veluna s'ouvre
...
settings-language-detail = La langue utilisée par Veluna dans l'interface
...
settings-typeface-detail = La police utilisée par Veluna dans l'interface
...
settings-notice = Copyright © 2026 Veluna Contributors. Veluna est fourni sans aucune garantie. C'est un logiciel libre, et vous êtes invité à le redistribuer selon les termes de la GNU General Public License version 3 ou ultérieure. Veluna n'est pas officiel et n'est pas affilié à Spotify AB.
...
toast-download-complete = { $name } téléchargé
toast-download-failed = Impossible de télécharger { $name }
...
update-available = Veluna { $version } est disponible
update-detail = Vous êtes en { $running }. Voyez ce qui a changé, ou mettez à jour maintenant.
update-detail-notes = Vous êtes en { $running }. Voyez ce qui a changé, puis mettez Veluna à jour comme vous l'avez installée.
...
settings-check-updates-detail = Demander une fois à GitHub au démarrage si une version plus récente existe. Veluna installe la mise à jour elle-même uniquement sous Windows ; ailleurs, elle vous montre ce qui a changé
