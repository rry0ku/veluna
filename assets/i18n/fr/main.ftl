...
update-detail-notes = Vous êtes en { $running }. Téléchargez la dernière version depuis GitHub pour mettre à jour.
...
settings-check-updates-detail = Demander à GitHub au démarrage si une nouvelle version est disponible
settings-check-now = Recherche manuelle de mise à jour
settings-check-now-detail = Rechercher la dernière version de Veluna sur GitHub
settings-check-now-btn = Vérifier maintenant
settings-checking-updates = Vérification…
toast-update-available = Mise à jour disponible : { $name }
toast-update-up-to-date = Vous êtes à jour
toast-update-failed = Impossible de vérifier les mises à jour
