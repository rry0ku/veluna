...
login-problem-premium = Veluna は Spotify Premium でストリーミングしますが、このアカウントは Premium プランに加入していません。続けるには Premium アカウントでサインインしてください。
...
login-usage-consent = Veluna の利用者数の推計にご協力ください。
login-device-code = { $url } でこのコードを入力してください
login-cookie-submit = 続行
login-cookie-hint = Cookie リクエストヘッダーをここに貼り付け
login-cookie-step-1 = music.youtube.com を開き、サインインしていることを確認します。
...
login-account-detail = このセッションは複数の Google アカウントにサインインしています。Veluna が使うアカウントを選んでください。
...
settings-saver-detail = Veluna が非アクティブの間アニメーションのフレームレートを制限する。次回起動時から適用
...
toast-download-complete = { $name } をダウンロードしました
toast-download-failed = { $name } をダウンロードできませんでした
