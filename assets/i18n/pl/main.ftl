...
login-problem-network = Veluna nie mogła połączyć się ze Spotify. Sprawdź połączenie internetowe i spróbuj ponownie.
...
login-problem-premium = Veluna odtwarza muzykę przez Spotify Premium, a to konto go nie ma. Zaloguj się na konto z Premium, aby kontynuować.
...
login-browser-detail = Veluna odczyta sesję YouTube Music z wybranej przeglądarki. Sesja pozostanie na tym urządzeniu.
...
login-account-detail = W tej sesji zalogowano więcej niż jedno konto Google. Wybierz to, którego ma używać Veluna.
...
settings-saver-detail = Ogranicz liczbę klatek animacji, gdy Veluna nie jest aktywna, od następnego uruchomienia
...
settings-language-detail = Język interfejsu Veluna
...
settings-license-detail = Powszechna Licencja Publiczna GNU v3.0 (GPL-3.0)
...
settings-notice = Copyright © 2026 Veluna Contributors. Veluna jest dostarczana bez żadnej gwarancji. To wolne i otwarte oprogramowanie, które możesz rozpowszechniać na warunkach Powszechnej Licencji Publicznej GNU wersja 3 (GPL-3.0). Veluna jest nieoficjalnym klientem i nie jest powiązana ze Spotify AB.
...
toast-download-complete = Pobrano { $name }
toast-download-failed = Nie udało się pobrać { $name }
...
update-available = Veluna { $version } jest dostępna
update-detail = Masz wersję { $running }. Zobacz, co się zmieniło, albo zaktualizuj teraz.
update-detail-notes = Masz wersję { $running }. Pobierz najnowsze wydanie z GitHuba, aby zaktualizować.
...
settings-check-updates-detail = Sprawdzaj na GitHubie przy starcie, czy jest nowa wersja
settings-check-now = Ręczne sprawdzanie aktualizacji
settings-check-now-detail = Sprawdź najnowszą wersję Veluny na GitHubie
settings-check-now-btn = Sprawdź teraz
settings-checking-updates = Sprawdzanie…
toast-update-available = Dostępna aktualizacja: { $name }
toast-update-up-to-date = Masz aktualną wersję
toast-update-failed = Nie udało się sprawdzić aktualizacji
